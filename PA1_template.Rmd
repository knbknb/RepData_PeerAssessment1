...
Preparing a new data frame, NA values removed:
```{r}
...
The maximum of the daily averages of the 5-minute intervals is n = `r max(on.weekend$mean_weekend_steps, on.workday$mean_workday_steps )`.


## Coping with missing data
Now we're plotting daily averages of steps made, using unfiltered data.  
To this end, we'll substitute NA values with the value for the median steps per day (`r sprintf("%0.0f", median_steps_per_day)`), determined above:
```{r}
bad <- is.na(act$steps)
...
makePlot(on.weekend, on.workday,"missing data replaced with daily averages")
